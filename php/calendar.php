...
if (isset($_GET['start_month'])){
  $start_month = date_create($_GET['start_month']);
}
...
if (!isset($start_month)){
  ...
  $start_month = date_create($current_year."/07/01");
}



$date_string = date_format($start_month,"m/d/Y");

$diff1Day = new DateInterval('P1M');
$diff2Day = new DateInterval('P2M');

$d1 = clone $start_month;
$d2 = clone $start_month->add($diff1Day);
$d3 = clone $start_month->add($diff1Day);

$months = array($d1,$d2,$d3);

echo '<input type="hidden" id="start_month" value="'.$date_string.'" >';
$bookeddays = 0;
for ($mIndex = 0; $mIndex<1; $mIndex++)
{
  $current_month = $months[$mIndex];
  
  echo "  <table class='cal'>   <caption>";

    echo "<span class='pointer prev' onclick='loadCalendar(-1);'> &larr;</span>";

    echo "<span class='pointer next' onclick='loadCalendar(1);'>&rarr; </span>";
         ...
         echo "<tr>";
          ...
          for ($j=0;$j<7;$j++)
          {
            ...
            else
            {
              ...
              $query = "SELECT * FROM reserveringen where (house_type = 1) AND start_datum < '".date_format($datePlus1,'Y-m-d H:i:s'). "' AND eind_datum > '".date_format($current_month,'Y-m-d H:i:s')."'";
