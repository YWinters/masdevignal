...
if (isset($_GET['start_month2'])){
  $start_month2 = date_create($_GET['start_month2']);
}
  
if (!isset($start_month2)){
  $month = date(m);
  if ($month > 8) {
    $current_year++;
  }
    $start_month2 = date_create($current_year."/07/01");
}
$date_string = date_format($start_month2,"m/d/Y");

$diff1Day = new DateInterval('P1M');
$diff2Day = new DateInterval('P2M');

$d1 = clone $start_month2;

$months = array($d1);

echo '<input type="hidden" id="start_month2" value="'.$date_string.'" >';
$bookeddays = 0;
for ($mIndex = 0; $mIndex<1; $mIndex++)
{
  ...
  echo " <article class='4u 12u(mobile) special'> <table class='cal'>   <caption>";
    ...
    echo "<span class='pointer prev' onclick='loadCalendar2(-1);'> &larr;</span>";

    echo "<span class='pointer next' onclick='loadCalendar2(1);'>&rarr; </span>";
         ...
         echo "<tr>";
          ...
          for ($j=0;$j<7;$j++)
          {
            ...
            else
            {
              ...
              $query = "SELECT * FROM reserveringen where (house_type = 1 OR house_type = 2) AND start_datum < '".date_format($datePlus1,'Y-m-d H:i:s'). "' AND eind_datum > '".date_format($current_month,'Y-m-d H:i:s')."'";
